      ...
      >
        ...
        <div className="container-premium relative z-10">
          ...
          >
            ...
            <p className="text-sm sm:text-sm md:text-base lg:text-lg xl:text-xl text-gray-700 max-w-2xl mx-auto mb-2">
              Discover what sets us apart as India's most trusted source for premium raw human hair.
            </p>
            <div className="body-premium text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl max-w-4xl mx-auto leading-relaxed">
              <div className={`${!showFullPhilosophy ? 'line-clamp-6 lg:line-clamp-none' : ''}`}>
                <strong>Emilio Beaufort</strong> is more than a luxury grooming brand—it's a movement built on trust, ethical sourcing, and the timeless beauty of <strong>temple hair</strong>. Our story began with a simple belief: that every person deserves to feel confident, authentic, and empowered by the products they use.
                <br /><br />
                We are proud to be a leading manufacturer and exporter of premium, ethically sourced raw Indian human hair, collected exclusively from verified South Indian temples. Our commitment to <strong>quality</strong> and <strong>integrity</strong> is unwavering—every bundle is natural, pure, and traceable to a single donor, with no acid washing or steam processing.
                <br /><br />
              ...
              <button
                ...
                className="mt-4 lg:hidden inline-flex items-center px-4 py-2 bg-black text-white rounded-lg transition-colors duration-300 text-sm sm:text-base font-semibold"
              >
                {showFullPhilosophy ? 'Read Less' : 'Read More'}
                <svg
                  className={`ml-2 w-4 h-4 transition-transform duration-300 ${showFullPhilosophy ? 'rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
          ...
          <div className="grid md:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
            {safeMap([
              ...
              <motion.div
                ...
                transition={{ duration: 1, delay: item.delay, ease: "easeOut" }}
                viewport={{ once: true }}
                className="text-center group mb-6 sm:mb-0"
              >
                <div className="flex justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-premium">
                  {item.icon}
                </div>
                <h3 className="heading-premium text-base sm:text-base md:text-lg lg:text-xl text-premium mb-3 sm:mb-4">{item.title}</h3>
                <p className="body-premium text-xs sm:text-sm md:text-base leading-relaxed">{item.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
